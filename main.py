...
class MenuHacked(object):
    ...
    def status_money(self, event=None):
        self.beepbeep()
        address = 0x00400000 + 0x000315B0
        offsets = [0x0]
        process = HackedProcess("gta-vc.exe", 99999999)
    ...
    def show_window(self):
        ...
        self.window.bind("<Escape>", self.status_exit)
        self.label_show.place(x=90, y=350)

        self.label_show = Label(self.window, text="☣ GTA VICE CITY ☣")
        self.label_show.config(bg="black", fg="yellow")
        self.label_show.configure(font=self.font)
        self.window.bind("<Escape>", self.status_exit)
        self.label_show.place(x=65, y=50)
